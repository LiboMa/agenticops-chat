...
- **Tiered models**: `bedrock_model_id` (Opus 4.6 default), `bedrock_model_id_cheap` (Haiku 4.5), `bedrock_model_id_strong` (Opus 4.6)
- **Prompt caching**: All agents use `CacheConfig(strategy="auto")` + `cache_tools="default"` for Bedrock prompt caching (~85% TTFT reduction, ~78% input cost savings). Toggle: `AIOPS_BEDROCK_CACHE_ENABLED`
...
| `config/mcp-servers.json` | MCP tool server definitions (standard mcpServers format, gitignored) |
...
|---------|---------|-------------|
| `bedrock_model_id` | `global.anthropic.claude-opus-4-6-v1` | Default model (Opus 4.6) |
...
| `bedrock_cache_enabled` | `true` | Prompt caching (cache_config=auto + cache_tools) |
