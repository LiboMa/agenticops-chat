...
**结构类**（本次只做与主题直接相关的两个）：① RCA 写 fix_plan 后 SRE 再写一份权威 FixPlan（双重修复作者，`rca_agent.py:133-134` + `metadata_tools.py:759`）；② scan_agent 携带 `check_health` 干 detect 的活，而其 docstring 又声明 "NOT FOR: health checks"（`scan_agent.py:42-44,107-145,154`）。**推迟**：sre_agent/sre_query 双重身份拆分、legacy orchestrator 重复栈（`analyze/rca.py` RCAEngine 等）删除 → 见 §6 Future。
...
1. **RCA 不再产 fix_plan**：RCA prompt 第 7 步改为只产出 root cause + 证据 + recommendations（修复规划全权归 SRE）；`save_rca_result` 的 `fix_plan/fix_risk_level` 参数保留兼容但从 prompt/docstring 移除；SRE 读 `RCAResult.recommendations`。消灭双重修复作者。
2. **scan_agent 移除 `check_health`**：从 scan 的工具列表和 prompt 删除（健康检查唯一入口 = detect，经 main 路由或 patrol）；`check_health` 函数本体若无其他引用则删除，有则保留但不再暴露给 scan。scan docstring 与行为终于一致。
...
| D6 | C：卫生 7 项 + 结构 2 项 + prompt-budget/路由测试更新 + CLAUDE.md·WORKFLOW.md 同步 |
