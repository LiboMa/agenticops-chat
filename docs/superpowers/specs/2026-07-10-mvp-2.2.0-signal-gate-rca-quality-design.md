# MVP-2.2.0 设计：Signal Gate 降噪 + RCA 质量五件套 + Agent 架构清理
...
3. **RCA 五件套全上** — 开启 thinking、证据门禁+置信度门控、Critic 对抗复核、人工反馈+失败回馈、**事件记忆注入**（同指纹/同类问题的历史 RCA 结论 + 修复结果，确定性注入 RCA 提示；借鉴 arXiv 2604.03933 的 Incident Memory 模式，见 3.6 与 5.5）。
...
- **轮数护栏**：`rca_max_iterations`（默认 40）经 Strands event_loop 上限传入——论文（2604.03933）给 AI 循环设 20 轮/15 万 token 预算的同款成本护栏；我们的 RCA 目前无任何轮数上限。
...
### 3.6 事件记忆注入（Incident Memory，借鉴 arXiv 2604.03933）

论文的核心机制之一：agent 维护 JSONL 事件记忆（事件签名 → 当时的根因 → 采取的修复 → 结果），新事件先与记忆匹配，命中即复用已验证的结论。我们的对应物**数据已经全有**（RCAResult 历史 + FixExecution 结果 + Signal Gate 的 fingerprint/issue_type），缺的只是把它接进 RCA 的输入：

- **实现（纯代码，零新表）**：`rca_service` 构造调用 prompt 时，按 `fingerprint_v2` 精确匹配 + `issue_type + resource_id` 次级匹配，取最近 ≤3 条**已终结**的同类 Issue 的（root_cause, critic_verdict, human_verdict, 修复方案摘要, 执行结果 success/fail），以 `INCIDENT MEMORY` 块确定性注入（与现有 TOPOLOGY CONTEXT 块并列，同样 2000 字符上限、fail-soft）。
- **信任规则**：注入块中明确标注每条的验证状态（human_verdict=correct 的排最前；disputed_by_execution 的标注"此结论曾被执行失败驳斥"）——历史结论是**先验**不是答案，prompt 要求 agent 必须用本次证据确认或推翻。
- **与 KB 案例搜索的关系**：不冲突。KB search_similar_cases 是跨资源的语义相似检索（LLM 主动调用）；Incident Memory 是同指纹/同类型的精确匹配（确定性注入，agent 无需选择就能看到）。前者广、后者准。
- 配置：`rca_incident_memory_enabled`（默认 true）、`rca_incident_memory_max`（默认 3）。
...
| `rca_incident_memory_enabled` | true | 同指纹/同类历史 RCA 结论注入 |
| `rca_incident_memory_max` | 3 | 注入条数上限 |
| `rca_max_iterations` | 40 | RCA 工具调用轮数上限（成本护栏，超限强制收敛保存） |
...
| D5 | B：rca-feedback API+UI + 失败回馈标记 + 超时看门狗/轮数护栏 + 状态机回边 + 事件 + Incident Memory 注入（§3.6）|
...
7b. 同 fingerprint 的 Issue 第二次触发 RCA → 调用 prompt 含 `INCIDENT MEMORY` 块（前次结论+验证状态可见）。
...
- **arXiv 2604.03933（ES Guardian Agent，2026-04）交叉验证**：该文的生产级自治 SRE agent（300 次自主调查-修复循环、18 小时跨系统故障自愈）与本设计的分层哲学一致——其五层监控中规则层零 LLM 处理 95% 的周期、LLM 只在灰区/深度调查介入（≈ 我们的 L1/L2 Signal Gate 与"确定性优先"铁律）；其 Incident Memory（签名匹配历史事件→复用已验证修复）被我们以 §3.6 事件记忆注入落地（零新表，复用 RCAResult+FixExecution+fingerprint）；其 AI 循环的 20 轮/150K token 预算护栏被我们以 `rca_max_iterations` 采纳；其 Safety Guard（执行前命令校验）我们已有等价物（skills/security.py 分级 + executor 审批门）。**有意不抄**：预测引擎（disk-fill/heap 趋势回归提前告警）属预防主题，与本周降噪+RCA 质量主线正交，且我们已有 graph patrol 承担预防位 → 记入 §6 Future。
...
8b. ~~effort（thinking）策略：后端升级 + 前端可覆盖~~ → **已落地 MVP-2.2.1**（`docs/superpowers/plans/2026-07-27-mvp-2.2.1-effort-policy.md`）。2.2.0 遗留问题是 `agent_rca_thinking_budget: 4096` 为未验证静态常量、且 chat 路径与自动管线被同一 YAML 键锁死。
9. 预测层（借鉴 2604.03933 预测引擎）：对 disk-fill/heap/容量类指标做零 LLM 趋势外推，"X 小时后满"提前建预防性 Issue（与 graph patrol 的 SPOF/容量检查合并成统一预防层）。
10. 预演修复（同论文 Phase 8 precomputed remediation）：高频 fingerprint 的已验证修复方案预关联到 Issue，审批通过即执行，省略重复 SRE 规划。
