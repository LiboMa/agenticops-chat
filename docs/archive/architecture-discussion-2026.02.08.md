...
## Session 5: 双通道 RCA Pipeline 架构 (2026-03-08)

### 架构概览

AgenticOps 提供两条**并行的、完全独立的告警接入通道**，共享同一套下游修复链路（RCA → SRE → Approve → Execute → Resolve）。

### Pipeline 1: Event-Driven（Webhook 直接集成）

```
① 监控系统 ──▶ ② AlertManager/SNS ──▶ ③ Webhook 接入 ──▶ ④ Auto-RCA
   (Prometheus/CW/DD)  (告警聚合)           (指纹去重)         (AI 分析根因)
                                                                 │
⑧ KB 更新 ◀── ⑦ Auto-Execute ◀── ⑥ Auto-Approve ◀── ⑤ Auto-SRE
  (案例沉淀)    (7步执行协议)      (L0/L1 自动审批)    (修复计划+风险评估)
```

**特点**：
- 直接 API 集成，无 IM Bot 依赖
- 低延迟、高吞吐
- 无 LLM 成本（解析为确定性逻辑）
- 适用于：监控系统直接对接、告警聚合网关、语义网关、第三方平台

**代码路径**：`parsers.py` → `alert_processor.py:process_alert()` → `rca_service.py:trigger_auto_rca()` → `pipeline_service.py`

**验证**：闭环验证 10/10 cases 全部通过。

### Pipeline 2: Channel-Driven（IM Agent 智能分诊）

```
监控系统 → 告警转发脚本/IM-Bot → IM 群组 → Main Agent (5步LLM分析) → create_health_issue → Auto-RCA → ...同上修复链
```

**关键架构事实**：IM 告警消息**不是**由监控系统直接发送的，而是由**告警转发脚本或 IM 机器人(Bot)**转发到 IM 群组中的。流程：

```
监控系统 (Prometheus/CW/Grafana)
    ↓
告警转发脚本 / IM Bot (AlertManager webhook receiver → Feishu API)
    ↓
IM 群组 (Feishu/DingTalk/WeCom/Slack)
    ↓
AgenticOps Agent (5步 LLM 分析 → create_health_issue → Auto-RCA)
```

**特点**：
- LLM 驱动的智能分诊，减少误报
- 支持对话上下文（SRE 可以在群里直接追问）
- 适用于：已有告警转发脚本/Bot 的环境
- 有 LLM 成本（每条告警消息需要 Agent 分析）

**代码路径**：`feishu_ws.py:_build_agent_input()` → Main Agent → `create_health_issue` tool → `rca_service.py` → same pipeline

### Pipeline Mode 配置

通过 `AIOPS_ALERT_PIPELINE_MODE` 环境变量控制活跃的 Pipeline：

| Mode | Webhook | IM Agent | 使用场景 |
|------|---------|----------|---------|
| `event_driven` | ✅ | ❌ | 纯 API 集成，无 IM 环境 |
| `channel_driven` | ❌ | ✅ | 纯 IM 集成，告警走 IM 群组 |
| `both` (默认) | ✅ | ✅ | 两条通道并行，最大灵活性 |

**防重复设计**：
- 如果同一告警同时通过两个通道进入，HealthIssue 指纹去重（SHA-256 of source+resource+title）会防止重复创建
- 建议：如果监控系统同时向 Webhook 和 IM 转发，选择 `event_driven` 或 `channel_driven` 以避免潜在的重复处理

### 两条 Pipeline 的定位

两条 Pipeline 是**一等公民**，保持告警接入接口的开放性和可扩展性：

- **Event-Driven**：对接监控系统、告警聚合网关、语义网关、第三方平台。无 IM Bot 依赖，低延迟，高吞吐，无 LLM 成本。
- **Channel-Driven**：对接已有告警转发脚本/Bot 的环境。LLM 驱动的智能分诊，支持对话上下文。
