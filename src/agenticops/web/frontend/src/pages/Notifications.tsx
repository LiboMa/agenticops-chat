  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-background rounded-lg shadow-lg w-full max-w-sm p-6">
        <h3 className="text-lg font-semibold text-foreground mb-2">Delete Channel</h3>
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <Badge className={styles[status] ?? "bg-secondary text-muted-foreground"}>
...
function IMBotsCard() {
  ...
  return (
    <Card>
      <CardHeader>
        <h2 className="text-lg font-semibold text-foreground">IM Bots</h2>
      </CardHeader>
      {isLoading ? (
...
function PlatformBadge({ platform }: { platform: string }) {
  ...
  return <Badge className={colors[platform] ?? "bg-secondary text-foreground"}>{platform}</Badge>;
}
...
function ChannelTypeBadge({ type }: { type: string }) {
  const colors: Record<string, string> = {
    ...
    dingtalk: "bg-sky-100 text-sky-700",
    wecom: "bg-emerald-100 text-emerald-700",
    webhook: "bg-secondary text-foreground",
  };
      ...
      r.severity_filter.length > 0 ? (
        <div className="flex flex-wrap gap-1">
          {r.severity_filter.map((s) => (
            <Badge key={s} className="bg-secondary text-muted-foreground">
              {s}
            </Badge>
          ))}
        </div>
