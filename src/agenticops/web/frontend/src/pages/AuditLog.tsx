...
const columns: Column<AuditLogEntry>[] = [
  {
    ...
    render: (r) => (
      <span className="text-sm text-[#9b9b9b] font-mono">{formatShortDate(r.timestamp)}</span>
    ),
  },
  {
    key: "action",
    header: "Action",
    render: (r) => (
      <span className="text-sm font-medium text-[#ececec]">{r.action}</span>
    ),
  },
  {
    key: "entity_type",
  ...
  },
  {
    key: "user_email",
    header: "User",
    render: (r) => <span className="text-sm text-[#9b9b9b]">{r.user_email}</span>,
  },
  {
    key: "details",
    header: "Details",
    render: (r) => (
      <span className="text-sm text-[#9b9b9b] truncate max-w-[250px] block">
        {r.details ?? "-"}
      </span>
    ),
  },
...
export default function AuditLog() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <h2 className="text-lg font-semibold text-[#ececec]">Audit Log</h2>
          <div className="flex items-center gap-3">
            <select
              value={action}
              onChange={(e) => setAction(e.target.value)}
              className="text-sm border border-[#424242] rounded-lg px-2 py-1.5 bg-[#2f2f2f] focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            >
            ...
            <div className="flex items-center gap-1">
              <label className="text-sm text-[#9b9b9b]">Hours:</label>
              <input
                type="number"
                min={1}
                max={720}
                ...
                className="w-16 text-sm border border-[#424242] rounded-lg px-2 py-1.5 bg-[#2f2f2f] focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              />
            </div>
          </div>
        </CardHeader>
