...
>
> **Version**: 2.5.0 · **Latest release**: [Cloud Security Review + skill wide loading (URL / git / zip), script sandbox and the Skills-page importer](docs/MVP-2.5.0-RELEASE.md) · **Full history** below · **Docs map**: [`docs/README.md`](docs/README.md).
>
> **Maturity:** the agent pipeline + CLI/Web/API are the validated core (10/10 closed-loop lab, see [Validation](#validation)). Cloud Security Review ran read-only against two real accounts, one in the China region — see the [E2E report](docs/MVP-2.5.0-E2E-REPORT.md). The one-click `deploy-sg` sandbox is exercised end-to-end; the `ec2`/`ecs`/`eks` Terraform stacks are scaffolding (see [Deployment](#deployment) for per-method maturity). Galaxy is an experimental PoC.
...
3. **Tiered models for cost.** The Claude 5 family where judgment matters most (Main routes on Opus 5, SRE plans on Fable 5.1), Opus 4.6 for the heaviest investigation and execution (RCA, Executor), Sonnet 4.6 for high-throughput work (Scan, Detect, Reporter), Haiku 4.5 as the economy tier. OpenAI models on Bedrock (gpt-oss, GPT-5.x) are first-class too; Anthropic-only features are gated per model family. Per-agent overridable; extended-thinking effort escalates automatically for critical or re-run RCAs and can be pinned per chat session. Token & cost is tracked per-call with real-time dashboards (Web + `aiops cost` CLI).
...
IM Bots ────────────┘        │                  SRE Agent     ──► Fix-plan generation (READ-ONLY)
                             ...
                             ├──► Security Engine (security/ — posture snapshots, CIS scoring, reachability)
...
Defaults from `config/settings.yaml` — the Claude 5 family for routing and planning (Opus 5, Fable 5.1), Opus 4.6 for the heaviest investigation + execution (RCA, Executor), Sonnet 4.6 for high-throughput work. The **safety spine**: only SRE plans, only Executor acts, and only through the approval gate — see [Auto-Fix Pipeline](#auto-fix-pipeline) and the [end-to-end workflow guide](docs/WORKFLOW.md).
...
- **Skills** (`skills/<name>/SKILL.md`) — agents can `add/improve/merge` skills via `skill_manage`, but writes land as **drafts only**. Packages imported from a URL / git repo / zip land as drafts too, stamped with provenance (`created_by=imported`, source, ref). `promote_skill` scans the **whole bundle** — SKILL.md plus any packaged `.sh`/`.py` — before publishing; human-authored skills are **pinned** and never auto-modified. All changes are versioned and recoverable.
...
| `AIOPS_SKILLS_SECURITY_SCAN_ON_PROMOTE` | `true` | Security-scan skills (whole bundle) before publishing |
...
├── services/     # Pipeline services (auto-fix, RCA + quality gate, Signal Gate, notifications, events, resolution)
...
├── web/          # FastAPI app + routers/ (webhooks, schedules, skills, security, signals, …) + React SPA (frontend/)
...
skills/           # 16 domain skill packages (+ draft/ staging) — SKILL.md + references/ (+ optional scripts)
...
User-facing workflow guide with Mermaid diagrams: [`docs/WORKFLOW.md`](docs/WORKFLOW.md). Codebase health audit + loop-engineering roadmap: [`docs/AUDIT-2026-06.md`](docs/AUDIT-2026-06.md). Full documentation map — living docs vs. dated snapshots, plus the bilingual (README / README_CN) maintenance rule: [`docs/README.md`](docs/README.md).
...
pytest tests/ -q                              # full suite: 4,300+ tests, ~2 min
